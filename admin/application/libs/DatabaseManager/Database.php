...
class Database
{
	 ...
	 */
	 public function multi_insert( $table, $head = false, $data = false, $arg = array() ){
		...
		foreach( $data as $dh => $dv ){
			...
			$v = '(';
				...
				foreach ( $dv as $vd ) {
					...
					if (isset($arg['row_format'])) {
						$ist_format = $arg['row_format'][$ist];

						if($ist_format == 'string') {
							$v .= "'".$vd."'";
						} else if($ist_format == 'number'){
							$v .= $vd;
						} else if($ist_format == 'boolean'){
							if( $vd ){
								$v .= 1;
							}else{
								$v .= 0;
		...
		$wk_step = 0;
		while ( $step_breaks >= 0 ) {
			//$query = ' <br><br>INSERT INTO '.$table.'(' . implode( ', ', $header ) . ') VALUES <br>'. implode( ",<br> ", $step_rows[$wk_step] )." " ;
